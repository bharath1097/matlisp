...
#+nil(defmacro-clause (FOR xa SLICING x ALONG axis &optional FROM start BELOW oend TO cend DOWNTO dend)
  (let* ((xa (if (listp xa) xa (list xa))))
    (when (or (and oend cend) (and dend (or cend oend))) (error "Use only one of BELOW TO DOWNTO."))
    (unless (null xa)
      (with-gensyms (dim sa ssym alist asym xlist xsym slst tmp-axis tmp-dim tmp-x x-eval)
	(let* ((n (length xa)))
	  `(progn
	     (with ,x-eval = ,x)
	     (with ,dim = -1)
	     (with ,sa = nil)
	     ,@(mapcan #'(lambda (x s) `((with ,x = nil))) xa)
	     (initially
