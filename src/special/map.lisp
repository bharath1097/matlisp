...
;;
(defmacro with-peeky! ((x &optional (step 1)) &rest body)
  (using-gensyms (decl (x step) (s))
    `(let (,@decl)
       (if-let (,s (gethash 'slice-increment (attributes ,x)))
	 (unwind-protect (progn
			   (incf (slot-value ,x 'head) (* ,step ,s))
			   ,@body)
	   (decf (slot-value ,x 'head) (* ,step ,s)))
...
(defmacro-clause (FOR xa SLICING x ALONG axis &optional FROM start BELOW oend TO cend DOWNTO dend)
  (let* ((xa (if (listp xa) xa (list xa))))
    ...
    (unless (null xa)
      (with-gensyms (dim ssym alist asym xlist xsym slst tmp-axis tmp-dim tmp-x x-eval)
	(let* ((n (length xa))
	  ...
	  `(progn
	     ...
	     (initially
	      (let* ((,alist ,(with-gensyms (axis-eval)
				  ...
				  ,axis-eval)))
		     (,xlist (let ((,x-eval (if (listp ,x-eval)
		...
		(setf ,@(mapcan #'(lambda (x s) `((values ,x ,s) (let* ((,tmp-axis (car ,alist))
									(,tmp-x (car ,xlist))
									(,ssym (if ,tmp-x (strides ,tmp-x ,tmp-axis) 0))
									(,xsym (when ,tmp-x
										 ,(with-gensyms (tmp)
										    `(let ((,tmp (slice~ ,tmp-x ,tmp-axis)))
										       (setf (gethash 'slice-increment (attributes ,tmp)) ,ssym)
										       ,tmp)))))
								   (values ,xsym ,ssym))
						  ,xlist (cdr ,xlist)
						  ,alist (cdr ,alist)))
