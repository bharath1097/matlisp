...
(defun dqaws
  ...
  (declare (type double-float a b alfa beta epsabs epsrel result abserr)
           (type integer4 integr neval ier limit lenw last)
  ...
  (prog ((lvl 0) (l1 0) (l2 0) (l3 0))
     ...
     (ftype
      (function
       (double-float double-float double-float double-float double-float
        integer4 double-float double-float integer4 double-float double-float
    ...
    (setf result 0.0d0)
    (setf abserr 0.0d0)
    (if (or (< limit 2) (< lenw (* limit 4))) (go label10))
    (setf l1 (+ limit 1))
    (setf l2 (+ limit l1))
    (setf l3 (+ limit l2))
    (multiple-value-bind
      ...
      (declare (ignore var-0 var-13 var-14 var-15 var-16 var-17))
      (when var-1 (setf a var-1))
      (when var-2 (setf b var-2))
      (when var-3 (setf alfa var-3))
      (when var-4 (setf beta var-4))
      (when var-5 (setf integr var-5))
      (when var-6 (setf epsabs var-6))
      (when var-7 (setf epsrel var-7))
      (when var-8 (setf limit var-8))
      (when var-9 (setf result var-9))
      (when var-10 (setf abserr var-10))
      (when var-11 (setf neval var-11))
      (when var-12 (setf ier var-12))
      (when var-18 (setf last var-18)))
    (setf lvl 0)
   label10
    (if (= ier 6) (setf lvl 1))
    (if (/= ier 0)
