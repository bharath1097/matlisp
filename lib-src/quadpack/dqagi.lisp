;;; Compiled by f2cl version 2.0 beta on 2001/02/23 at 10:10:17
;;; 
...
(defun dqagi
       (f bound inf epsabs epsrel result abserr neval ier limit lenw last iwork
        work)
  (declare (type double-float bound epsabs epsrel result abserr)
           (type integer4 inf neval ier limit lenw last)
  ...
  (prog ((lvl 0) (l1 0) (l2 0) (l3 0))
    ...
    (multiple-value-bind
      ...
      (when var-1 (setf bound var-1))
      (when var-2 (setf inf var-2))
      (when var-3 (setf epsabs var-3))
      (when var-4 (setf epsrel var-4))
