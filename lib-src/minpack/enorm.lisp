...
(let ((one 1.0d0)
  ...
  (defun enorm (n x)
    (declare (type (array double-float (*)) x) (type integer4 n))
    (prog ((agiant 0.0d0) (floatn 0.0d0) (s1 0.0d0) (s2 0.0d0) (s3 0.0d0)
      ...
      (setf x1max zero)
      (setf x3max zero)
      (setf floatn (coerce n 'double-float))
      (setf agiant (/ rgiant floatn))
      (fdo (i 1 (+ i 1))
           ((> i n) nil)
           (tagbody
             (setf xabs (dabs (fref x (i) ((1 n)))))
      ...
      (setf enorm (* x1max (dsqrt (+ s1 (/ (/ s2 x1max) x1max)))))
      (go label130)
     label100
      (if (= s2 zero) (go label110))
      (if (>= s2 x3max)
          (setf enorm (dsqrt (* s2 (+ one (* (/ x3max s2) (* x3max s3)))))))
      (if (< s2 x3max)
          (setf enorm (dsqrt (* x3max (+ (/ s2 x3max) (* x3max s3))))))
      (go label120)
     label110
      (setf enorm (* x3max (dsqrt s3)))
     label120
     label130
      (go end_label)
      '""
