...
(defun plot (data &key (style "lines") (color #xffffff))
  #+nil(declare (type (and real-numeric-tensor (or tensor-matrixp tensor-vectorp)) data))
  (let* ((color (if (listp color) color (let ((lst (list color))) (setf (cdr lst) lst)))))
    (labels ((ppair (s x y color style)
	       (letv* (((r g b) (split-color color))
		       (*print-max-len* t))
		 (format s "'-' using 1:2 with ~a linecolor rgb(~a, ~a, ~a)~%" style r g b)
		 (print-tensor (apply #'tensor-append 1 (mapcar #'(lambda (x) (suptensor~ x 2)) (list x y))) s)
		 (format s "e e~%"))))
      (with-gnuplot-stream (s)
	(format s "plot ") (ppair s (car data) (cadr data) #xffffff style))
      )))
